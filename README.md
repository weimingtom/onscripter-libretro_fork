...
(All platforms alpha) https://github.com/weimingtom/onscripter-libretro_fork/tree/master/out_bin/build_yuri_20250521  
(alpha) https://github.com/weimingtom/onscripter-libretro_fork/tree/master/out_bin/build_yuri_20250426   
...
## Bugs or TODO  
* (20250907) Known problems: If the file path contains Chinese, the RetroArch will crash. Make the path all full English.
...
yuri版快进到开头某段偶尔可能会闪退；  
开头有一段对话对话框左上角不显示说话人（例如“魔女”），但yuri版没这个bug；  
（20250320 fixed, see FIX_TACHI_IMAGE_PNG_MASK_BUG）一些图片被当作带遮罩立绘显示导致少了右半边一半；  
```
关于上次修复onscripter RA核心（注：我不是这个核心的原作者）对于立绘背景不透明的问题，
通过图片右半边的遮罩实现立绘透明背景效果，其实我测试过是有问题的——它会导致
其他的少数游戏的立绘出现别的问题（例如魔女的花园），
因为它会错误把一些立绘图片横向对半切走一半，
...
昨天我尝试再次修复onscripter RA核心（我不是核心的作者）的遮罩立绘问题，
方法是在所有逻辑的最后加上一段判断右半边
（实际上程序是判断左半边，原因不明）图片是否都是黑白灰三种色，
如果不是的话当作非遮罩立绘处理，
如果是的话当作遮罩立绘处理，暂时似乎可以解决问题，
避免其他游戏的图片被错误当作带遮罩的立绘来显示
（实际是会被当作24色不带透明通道的图片处理导致显示错误）
```
* 遥仰凰华：  
主菜单上下切换菜单时显示文字错位（yuri版也一样）；
(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 读取进度菜单最后-号错误折行（yuri版没折行）；  
下方文本框左侧边缘有一条垂直向下的一段绿色线（有时候整个左侧不含黑边部分的左边缘都是绿色，yuri版也一样）；  
不确定是否有乱字情况  
* 苍蓝眼瞳的人偶：  
(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 读取进度菜单最后-号错误折行（yuri版没折行）；  
读档菜单似乎无法向下循环到顶，不过好像又没出现这个bug了，可能已经修复  
* 精爆双姬（这标题谁起的？）：  
(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 读档菜单最后-号错误折行（yuri版没折行），  
正文最开始那句话有个方块字（yuri版也是），不过后面似乎没太多乱字情况（不确定，待考）    
* 秽翼：
读档似乎有乱码
(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 和最后-号错误折行（yuri版没折行），  
字体可能太小了，  
屏幕下方黑边（顶部对齐）；  
...
如果用onsyuri或者其他onscripter版都无法解决（立绘背景不透明）。
我可以拿到正确版本对应的onscripter-en源码，
至于是否能解决，目前还不好说 ​​​

onscripter立绘遮罩的bug我已经找到方法修复了，方法是把onscripter-en
（非常旧的版本，在2009年左右）里面loadImage()后半部分的增加的代码搬过来即可（有两处）。
有时间我会把修改更新到onscripter-libretro_fork。
这个bug在上游最新版和onsyuri中也存在，
只有很少中文版本和所有onscripter-en修复了此bug

tachi_image_png_mask_bug.txt
...
* 寒蝉：
按Y跳过开头OP，到正文开始等待输入的动画图标底色是紫色；  
(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 按X跳到读取菜单最后的-号被回车折行了；  
yuri版也是等待图标紫色背景，但读取菜单的-号正常（不错误折行）  
```
关于onscripter出现加载界面回车折行的bug，
我查到应该是我融合onscripter和onscripter-jh代码时造成的bug
（原版onscripter-jh不会出现这个bug），
我尝试通过修改num_xy数组（可能是用于存储每一行最多显示的字数）
来防止提前折行换行。可能另一个bug退出对话框的问号折行也是
因为这个bug导致的，待考，这个bug是否已经改好也待考，
因为改这个bug影响太大了，要等以后有时间再详细测试，
或者尝试把文本外框的矩形用红色线画出来。
这个bug修改我也会很快提交
（注：上述修改有bug，会导致所有文本的折行规则出问题）
debug_load_menu.txt  

我重新修复自己魔改版的onscripter RA核心的加载菜单文本折行bug
（其实包括保存菜单和确定退出菜单和确定菜单），我这次不改FontInfo.cpp了，
而是直接改ONScripter_rmenu.cpp，因为我发现可以修改num_xy[0]和setXY
来微调文本框的宽度和文本框的左位置，
如果只改这里可以避免改变所有文本框的折行规则
see ONScripter_rmenu.cpp, debug_load_menu.txt, FIX_LOAD_MENU_LINE_WRAP_BUG
（20250316重新修复）
```
...
* 待考：(20250307 fixed, see FIX_LOAD_MENU_LINE_WRAP_BUG) 退出问号有回车；yuri版没这个问题     
...
* 秋之回忆2: (20250322 fixed, see FIX_MEMORIES_OFF_2ND_SYS_BMP_BUG)所有版本，首页菜单-图片鉴赏-选上下进入，缩略图会花屏；似乎无法通过方向键切换到旁边的缩略图；
```
onscripter RA核心（我不是核心的作者）修改，
尝试修复秋之回忆2的画册缩略图的bug，勉强改成这样。
造成缩略图雪花屏的原因是把32色强行放到24色bmp中，
导致后面有一部分数据丢失了，
我把丢失的数据用白色填充，
所以修复bug后上半部分的图片是白色的，
我也没办法 ​​​

(TODO) 理论上可以设置黑色或者白色作为透明色，但暂时还没尝试修改，
所以修复后仍然会有白色或黑色背景色，这个问题可能还能继续改进
```
